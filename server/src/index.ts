...
app.use(
  cors({
    origin: (origin, callback) => {
      if (origin === "https://administracion.serviredgane.cloud" || origin === "http://localhost:3000") {
        callback(null, true); // Permitir esta origen
      } else {
        callback(new Error("no tienes permitido el acceso a este recurso"));
      }
    },
  }),
);
...
app.use(
  (
    ...
    if (err instanceof SyntaxError && "body" in err) {
      ...
      return res
        .status(400)
        .json({ error: "JSON inválido", detalle: err.message });
    }
    next(err);
  },
);
